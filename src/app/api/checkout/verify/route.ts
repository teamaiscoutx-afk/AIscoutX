...
/**
 * POST /api/checkout/verify — verifies Razorpay payment signature and upgrades to Pro.
 */
export async function POST(request: Request) {
  try {
    ...
    const provisioned = await provisionProSubscription({
      userId: user.id,
      email: user.email,
      razorpayCustomerId: subscriptionId ?? orderId ?? null,
    });
    ...
    logServerError("checkout.verify", err);
    return NextResponse.json(
